...
async function run() {
	...
	fileInput?.addEventListener('change', async (event) => {
		...
		currentRawImageState.set(currentRawImage);
...
export function renderImageData(rawImage) {
	...
	const imageData = pictureCanvasCtx?.createImageData(rawImage.width, rawImage.height);
